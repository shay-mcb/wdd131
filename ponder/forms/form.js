...
function updateNotesField() {
  ...
  if(value === "many") { // this function makes it so that the box shows up when two or more is chosen and disappears otherwise
    notesContainer.hidden = false;
    notes.required = true; // makes this a required input field
  } else {
...
function getSelectedCampuses() {
  ...
  return Array.from(campusBoxes)
    .filter(box => box.checked) // only looks at the campuses that have been checked
    .map(box => box.value); 
}
...
form.addEventListener("submit", function (event) {
  ...
  if(type === "many" && note.length == 0) {
    output.textContent = "Please write a note";
    return;
  }
